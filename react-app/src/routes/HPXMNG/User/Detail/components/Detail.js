import React from 'react'
import { IndexLink, Link, withRouter, browserHistory } from 'react-router'
import moment from 'moment'

import { postReq } from '../../../../../core/fetch'

import './Detail.css'

import { 
  Form, 
  Input, 
  Tooltip, 
  Icon, 
  Cascader, 
  Select, 
  Row, 
  Col, 
  Checkbox, 
  Button, 
  InputNumber, 
  DatePicker, 
  Upload, 
  message, 
  Modal 
} from 'antd';

import { precisionValidator } from '../../../../../core/antdFormUtil.js'

import { checkPhone } from '../../../../../core/util.js'

const FormItem = Form.Item;
const Option = Select.Option;

class DetailForm extends React.Component {
  state = {enterpriseData: '请选择',
    smsButtonDisabled: false,//发送验证码按钮是否可编辑
    smsButtonMsg: '获取验证码',
    previewVisible: false,//图片预览是否可见
    previewImage: '',//预览图片地址
    intervalId: null,
      current:this.props.name.value,
  };
  componentWillUnmount(){
    clearInterval(this.state.intervalId)
  }
  handleSubmit = (e) => {
    e.preventDefault();
    const self = this;
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        console.log('Received values of form: ', values);
        //在提交前格式化部分数据
        values.latestLoginTime = values.latestLoginTime ? values.latestLoginTime.format('YYYY-MM-DD') : '';
      
        values.lockTime = values.lockTime ? values.lockTime.format('YYYY-MM-DD') : '';
      
        values.registerTime = values.registerTime ? values.registerTime.format('YYYY-MM-DD') : '';
      
        values.createTime = values.createTime ? values.createTime.format('YYYY-MM-DD') : '';
      
        values.modifiedTime = values.modifiedTime ? values.modifiedTime.format('YYYY-MM-DD') : '';
      
        self.props.saveSubmit(values,self.props.id,values.enterpriseName,values.name);
      }
    });
  }
  checkPhoneConfirm = (phoneKey, rule, value, callback) => {
    const form = this.props.form;
    if (!checkPhone(form.getFieldValue(phoneKey))) {
      callback('手机号格式不正确!');
    } else {
      callback();
    }
  }
  //发送验证码
  fetchCaptcha = async (smsCodeKey,phoneKey) => {
    const form = this.props.form;
    form.setFields({
        [smsCodeKey]:{
          value: this.props[smsCodeKey].value,
          errors:null,
        }
      })
    if(!form.getFieldValue(phoneKey)||!checkPhone(form.getFieldValue(phoneKey))){
      form.setFields({
        captcha:{
          errors:[new Error('请输入正确格式的手机号!')],
        }
      })
    } else {
      const resp = await postReq(__PROXY__ + '/trade/testOb/smsCode',{[phoneKey]:form.getFieldValue(phoneKey)});
      const result = await resp.json();
      if( result.success ){
        message.success('成功发送验证码');
        let self = this;
        let number = 59;
        let intervalId = setInterval(
          () => {
            self.setState({
              smsButtonDisabled: true,
              smsButtonMsg:'重新获取验证码('+number+')'
            });
            number--;
            if(number == 0) {
              self.setState({
                smsButtonDisabled: false,
                smsButtonMsg:'获取验证码'
              });
              clearInterval(intervalId);
            }
          },1000
        );
        self.setState({intervalId: intervalId})
      }
    }
  }
  beforeUpload(file) {//文件上传前的回调
    const isJPG = file.type === 'image/png' || file.type === 'image/jpeg';
    if (!isJPG) {
      message.error('你只能上传JPG 或PNG 图片!');
    }
    const isLt2M = file.size / 1024 / 1024 < 2;
    if (!isLt2M) {
      message.error('图片必须小于 2MB!');
    }
    return isJPG && isLt2M;
  }
  normFile = (info) => {
    //如果希望组件能提供查看功能，则需要设置url字段的值
    return info.fileList;
  }
  handleCancel = () => this.setState({ previewVisible: false })
  handlePreview = (file) => {
    this.setState({
      previewImage: file.url || file.thumbUrl,
      previewVisible: true,
    });
  }
  render() {
    const { previewVisible, previewImage } = this.state;
    const { getFieldDecorator } = this.props.form;
    const id = this.props.id || null;
    const formItemLayout = {
      labelCol: {
        xs: { span: 24 },
        sm: { span: 6 },
      },
      wrapperCol: {
        xs: { span: 24 },
        sm: { span: 14 },
      },
    };
    const tailFormItemLayout = {
      wrapperCol: {
        xs: {
          span: 24,
          offset: 0,
        },
        sm: {
          span: 14,
          offset: 6,
        },
      },
    };
    const prefixSelector = getFieldDecorator('prefix', {
      initialValue: '86',
    })(
      <Select className="icp-selector mobileBefore">
        <Option value="86">+86</Option>
      </Select>
    );

   const optionOne = this.props.enterpriseList.data;

      console.log('optionOne',optionOne)

   const provinceOptions = optionOne.map(v => <Option  key={v.id}>{v.name}</Option>);
    return (
      <Form onSubmit={this.handleSubmit}>
        <FormItem
            key="enterpriseName"
            {...formItemLayout}
            label="所属公司"
            hasFeedback
            className={id?"edit":"add"}
        >
            {getFieldDecorator('enterpriseName', {

                rules: [
                    { required: true, message: '请选择!'},
                ],
            })(
           <Select
                showSearch
                style={{ width: 200 }}
                optionFilterProp="children"
            >
             {provinceOptions}
            </Select>

               /* <Select showSearch
                        placeholder="请选择" >
                    {optionOne.map(
                        (v) => <Option key={v.name} value={v.value}>{v.name}</Option>
                    )}
                </Select>*/

            )}
        </FormItem>
        <FormItem
          key="email"
          {...formItemLayout}
          label="邮箱"
          hasFeedback
        >
          {getFieldDecorator('email', {
            rules: [{
              type: 'email', message: '请输入邮箱格式的数据!',
            }, {
              required: true, message: '请输入你的邮箱!',
            }],
          })(
            <Input autoComplete="off"/>
          )}
        </FormItem>
      
        <FormItem
          key="name"
          {...formItemLayout}
          label="用户名称"
          hasFeedback
        >
          {getFieldDecorator('name',{
            rules: [
              { required: true, message: '请输入数据!', whitespace: true },
              {
                 message: '长度需在0-65536位!',
              },
            ],
          })(
            <Input 
            />
          )}
        </FormItem>
      
        <FormItem
          key="gender"
          {...formItemLayout}
          label="性别"
          hasFeedback
        >
          {getFieldDecorator('gender',{
            rules: [
              { required: true, message: '请选择!'},
            ],
          })(
            <Select placeholder="请选择" >
              {[{"value":"M","name":"男"},{"value":"F","name":"女"}].map(
                (v) => <Option key={v.value} value={v.value}>{v.name}</Option>
              )}
            </Select>
          )}
        </FormItem>
      
        <FormItem
          key="phoneBinded"
          {...formItemLayout}
          label="是否绑定手机"
          hasFeedback
        >
          {getFieldDecorator('phoneBinded',{
            rules: [
              { required: true, message: '请选择!'},
            ],
          })(
            <Select placeholder="请选择" >
              {[{"value":"T","name":"是"},{"value":"F","name":"否"}].map(
                (v) => <Option key={v.name} value={v.value}>{v.name}</Option>
              )}
            </Select>
          )}
        </FormItem>
      
        <FormItem
          key="phone"
          {...formItemLayout}
          label="手机号码"
        >
          {getFieldDecorator('phone', {
            rules: [
              { required: true, message: '请输入你的手机号!' },
              { validator: this.checkPhoneConfirm.bind(this,'phone') }
            ],
          })(
            <Input addonBefore={prefixSelector} />
          )}
        </FormItem>
      
        <FormItem
          key="position"
          {...formItemLayout}
          label="职位"
          hasFeedback
        >
          {getFieldDecorator('position',{
            rules: [
              { required: true, message: '请输入数据!', whitespace: true },
              {
                 message: '长度需在0-65536位!',
              },
            ],
          })(
            <Input 
            />
          )}
        </FormItem>
      
        <FormItem
          key="enabled"
          {...formItemLayout}
          label="是否可用"
          hasFeedback
        >
          {getFieldDecorator('enabled',{
            rules: [
              { required: true, message: '请选择!'},
            ],
          })(
            <Select placeholder="请选择" >
              {[{"value":"T","name":"是"},{"value":"F","name":"否"}].map(
                (v) => <Option key={v.name} value={v.value}>{v.name}</Option>
              )}
            </Select>
          )}
        </FormItem>
      
        <FormItem
          key="locked"
          {...formItemLayout}
          label="是否锁定"
          hasFeedback
        >
          {getFieldDecorator('locked',{
            rules: [
              { required: true, message: '请选择!'},
            ],
          })(
            <Select placeholder="请选择" >
              {[{"value":"T","name":"是"},{"value":"F","name":"否"}].map(
                (v) => <Option key={v.name} value={v.value}>{v.name}</Option>
              )}
            </Select>
          )}
        </FormItem>

      
        <FormItem {...tailFormItemLayout}>
          <Button 
            type="primary" 
            htmlType="submit" 
            size="large" 
            loading={this.props.saveStatus.loading}>
            提交
          </Button>
        </FormItem>
        <Modal visible={previewVisible} footer={null} onCancel={this.handleCancel}>
              <img alt="example" style={{ width: '100%' }} src={previewImage} />
        </Modal>
      </Form>
    );
  }
}

const WrappedDetailForm = Form.create({
    onFieldsChange(props, changedFields) {
    props.onChange(changedFields);//调用父组件的更新方法
  },
  mapPropsToFields(props) {
    var arr = [
        {
            key: 'enterpriseName',
            value: {
                ...props.enterpriseName
            }
        },
        {
          key: 'email',
          value: {
            ...props.email
          }
        },{
            key: 'id',
            value: {
                ...props.id
            }
        },{
          key: 'name',
          value: {
            ...props.name
          }
        },{
          key: 'gender',
          value: {
            ...props.gender
          }
        },{
          key: 'secretKey',
          value: {
            ...props.secretKey
          }
        },{
          key: 'phoneBinded',
          value: {
            ...props.phoneBinded
          }
        },{
          key: 'phone',
          value: {
            ...props.phone
          }
        },{
          key: 'position',
          value: {
            ...props.position
          }
        },{
          key: 'latestLoginTime',
          value: {
            ...props.latestLoginTime,
            value: props.latestLoginTime && props.latestLoginTime.value ? moment(props.latestLoginTime.value) : null,
          }
        },{
          key: 'enabled',
          value: {
            ...props.enabled
          }
        },{
          key: 'locked',
          value: {
            ...props.locked
          }
        },{
          key: 'lockTime',
          value: {
            ...props.lockTime,
            value: props.lockTime && props.lockTime.value ? moment(props.lockTime.value) : null,
          }
        },{
          key: 'registerCode',
          value: {
            ...props.registerCode
          }
        },{
          key: 'registerTime',
          value: {
            ...props.registerTime,
            value: props.registerTime && props.registerTime.value ? moment(props.registerTime.value) : null,
          }
        },{
          key: 'createTime',
          value: {
            ...props.createTime,
            value: props.createTime && props.createTime.value ? moment(props.createTime.value) : null,
          }
        },{
          key: 'modifiedBy',
          value: {
            ...props.modifiedBy
          }
        },{
          key: 'modifiedTime',
          value: {
            ...props.modifiedTime,
            value: props.modifiedTime && props.modifiedTime.value ? moment(props.modifiedTime.value) : null,
          }
        }];
    var obj = {};
    arr.forEach(
      (v)=> obj[v.key] = v.value
    )
    return obj;
  },
  onValuesChange(_, values) {
    console.log(values);
  },
})(DetailForm);

class Detail extends React.Component {
  constructor(props){
    super(props);
  }
  componentWillMount(){
      var eid =this.props.location.query.eid;
    this.props.fetchDetail(this.props.params.id,eid);
    //调用module方法加载数据
      this.props.fetchEnterpriseList();
  }
  componentDidMount(){
    
  }
  componentWillReceiveProps(nextProps){
    const { saveStatus, fetchDetailStatus, err } = nextProps;
    if(
      this.props.saveStatus.type != saveStatus.type 
      && saveStatus.type.match('SUCCESS')
    ){//保存成功，返回上一页，这里没对上一页为空的情况做处理
      message.success('保存成功');
      browserHistory.go(-1);
    }
    if (err) {//拉取列表失败，提示错误信息
      message.error(err);
      this.props.clearErr();
    }
  }
  render() {
    return  <div><WrappedDetailForm
                  {...this.props.fields}  
                  saveStatus={this.props.saveStatus}
                  fetchDetailStatus={this.props.fetchDetailStatus}
                  onChange={this.props.updateFields}
                  saveSubmit={this.props.save}
                  enterpriseList={this.props.enterpriseList}
                  name={this.props.fields.name}
                  id={this.props.params.id}/>
            </div>
  }
}

Detail.propTypes = {
  // increment   : React.PropTypes.func.isRequired
}

export default withRouter(Detail)
