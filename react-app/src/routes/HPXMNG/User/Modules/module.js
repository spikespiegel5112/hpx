...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'userModule::';
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().userModule.list;
...
export  const fetchDetail =  (id,eid) => {
  return async (dispatch, getState) => {
    if(!id){//没有id，就是新增
      dispatch({
        type: UPDATE_FIELDS,
    ...
    try{
       console.log('bbbbbbbbbbbbbb',eid)
      const resp = await getReq(
        '/core'+'/core/api/v1/palt/enterprise/'+eid+'/users/'+id,
        {
        }
      )
      const result = await resp.json();
...
export  const save =  (values, id,eid) => {
  return async (dispatch, getState) => {
    dispatch({
      type    : SAVE_REQUEST,
    })
