...
export default (store) => ({
  ...
  getComponent (nextState, cb) {
    ...
    require.ensure([], (require) => {
      ...
      injectReducer(store, { key: 'warningmodelModule', reducer : reducer1 })
  ...
  getChildRoutes (location, cb) {
    const routes = [
      require('./Check').default(store),
      require('./Add').default(store),		
      require('./Redact').default(store),
