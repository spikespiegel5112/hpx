...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'signatureManageModule::';
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await getReq(
        '/core'+'/core/api/v1/venterprise/'+ eid +'/esigns',
        {...value}
      )
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await patchReq(
        '/core' + '/core/api/v1/venterprise/esigns/' + id + '?name=' + name + '&enabled=' + enabled,
      )
      dispatch({
        type : ABLED_SUCCESS,
        payload : resp
  ...
  return async (dispatch, getState) => {
    ...
    try{
      let resp = await deleteReq(
        '/core' + '/core/api/v1/venterprise/'+ eid +'/esigns/' + id,
      )
      dispatch({
        type : DELETE_SUCCESS,
      })
