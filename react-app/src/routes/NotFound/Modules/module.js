...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'addressModule::';
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().addressModule.list;
    ...
    try{
      const resp = await getReq(
        '/proxyBiMai'+'/huaqian/api/v1/address/search',
        queryParams
      )
...
export  const fetchDetail =  (id) => {
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await getReq(
        '/proxyBiMai'+'/huaqian/api/v1/address/get/'+id,
        {
        }
      )
      const result = await resp.json();
    ...
    ,
    addressName: {
      value: data.addressName || null
    }
    ,
    isDefault: {
      value: data.isDefault || null
    }
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await deleteReq(
        '/proxyBiMai'+'/huaqian/api/v1/address/delete/'+id,
        {
        }
      )
      // const result = await resp.json();
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = 
        id 
        ? await patchReq(
            '/proxyBiMai'+'/huaqian/api/v1/address/update/'+id,
            {
                ...values,
            }
          )
        : await putReq(
            '/proxyBiMai'+'/huaqian/api/v1/address/add',
            {
                ...values,
                id: id || '',
            }
