...
class Compo extends React.Component {
  constructor(props) {
    ...
    this.columns = [{
        ...
        render: (text, record) => (
          <span>
            <Popconfirm title="确定更改启/停用状态?" onConfirm={() => this.props.updateState(record.id)}>
              <a href="#">{record.available == 'T'? '禁用': '启用' }</a>
            </Popconfirm>
  ...
  componentWillReceiveProps(nextProps) {
    const { deleteStatus, listStatus, updateStateStatus, fetchList, list, err } = nextProps;
    if (
    ...
    if (
      this.props.updateStateStatus.type != updateStateStatus.type
      && updateStateStatus.type.match('SUCCESS')
    ) {//删除成功重新拉取列表数据
      fetchList();
    }
  ...
  search() {
    ...
    let arr = [{
        key: 'available',
        value: {
          value: queryParams.available ? queryParams.available.value : null
        }
      },{
        key: 'code',
        value: {
  ...
  getUrlQueryParams(v) {
    ...
    return '/hpx2/hpxmng/proudct/' + encodeURI(JSON.stringify(v || {}));
  }
  render() {
