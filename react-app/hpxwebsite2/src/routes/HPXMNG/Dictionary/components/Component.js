...
class Compo extends React.Component {
  constructor(props) {
    ...
    this.columns = [{
        ...
        render: (text, record) => (
          <span>
            <Link to={`${this.getUrlQueryParams()}/detail/${record.id}?code=${record.code}`}>
              修改
            </Link>
            <span className="ant-divider" />
            <Popconfirm title="确定删除?" onConfirm={() => this.props.del(record.code)}>
              <a href="#">删除</a>
            </Popconfirm>
          </span>
        ),
  ...
  search() {
    ...
    let arr = [{
        ...
        key: 'parentCode',
        value: {
            value: queryParams.parentCode ? queryParams.parentCode.value : null
        }
    }];
    let obj = {};
  ...
  getUrlQueryParams(v) {
    ...
    return '/hpx2/hpxmng/dictionary/' + encodeURI(JSON.stringify(v || {}));
  }
  render() {
