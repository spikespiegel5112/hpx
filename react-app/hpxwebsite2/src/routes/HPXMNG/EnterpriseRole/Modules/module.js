...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'enterpriseRoleModule::';
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().enterpriseRoleModule.list;
    ...
    let productCode = queryParams.productCode ? queryParams.productCode.value : null;
    if(productCode) queryParams.productCode = queryParams.productCode.value;
    else delete queryParams.productCode;
    ...
    try{
      const resp = await getReq(
        '/core'+'/core/api/v1/products/ent_roles/roles',
        queryParams
      )
...
export  const fetchDetail =  (id) => {
  return async (dispatch, getState) => {
    ...
    try{
      const resp = await getReq(
        '/core'+'/core/api/v1/products/{procode}/ent_roles/{ercode}/roles/'+id,
        {
        }
      )
      const result = await resp.json();
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = 
        ...
        : await putReq(
            '/core'+'/core/api/v1/products/{procode}/ent_roles/{ercode}/roles',
            {
                ...values,
                id: id || '',
            }
...
export const updateFields = (fields,type) => {
  return (dispatch, getState) => {
    dispatch({
      type: UPDATE_FIELDS,
      payload: {
        ...getState().enterpriseRoleModule.fields,
        ...fields
      }
    })
  }
