...
// Constants
// ------------------------------------
const ACTION_PREFIX = 'moduleNameHook::';
...
export const fetchList =  () => {
  return async (dispatch, getState) => {
    ...
    let list = getState().moduleNameHook.list;
    let queryParams = Object.assign({},list.queryParams);
    queryTransSearchHook
  ...
  return async (dispatch, getState) => {
    ...
    try{
      const resp = 
        id 
        ? await patchReq(
            'proxyHook'+'updateUrlHook/'+id,
            {
                ...values,
            }
          )
        : await putReq(
            'proxyHook'+'addUrlHook',
            {
                ...values,
                id: id || '',
            }
...
export const updateFields = (fields,type) => {
  return (dispatch, getState) => {
    dispatch({
      type: UPDATE_FIELDS,
      payload: {
        ...getState().moduleNameHook.fields,
        ...fields
      }
    })
  }
